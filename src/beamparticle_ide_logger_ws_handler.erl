...
%%% %CopyrightEnd%
%%%-------------------------------------------------------------------
-module(beamparticle_ide_logger_ws_handler).
-author("neerajsharma").
...
websocket_init(State) ->
    ...
    lager:debug("init beamparticle_ide_logger_ws_handler websocket"),
    {ok, State}.
...
run_query(#{<<"id">> := Id,
            <<"method">> := <<"getLogLevel">>,
            <<"params">> := _Value} = _QueryJsonRpc, State) ->
    ResponseJsonRpc = #{
      <<"jsonrpc">> => <<"2.0">>,
      <<"id">> => Id,
      <<"result">> => 30}, %% TODO what should this be for more debugging? null?
    Resp = jiffy:encode(ResponseJsonRpc),
    {reply, {text, Resp}, State, hibernate};
run_query(#{<<"id">> := Id} = QueryJsonRpc, State) ->
    lager:info("[~p] IDE Log = ~p", [self(), QueryJsonRpc]),
    ResponseJsonRpc = #{
      <<"jsonrpc">> => <<"2.0">>,
      <<"id">> => Id,
      <<"result">> => null},
