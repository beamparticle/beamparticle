...
%%% %CopyrightEnd%
%%%-------------------------------------------------------------------
-module(beamparticle_ide_workspace_ws_handler).
-author("neerajsharma").
...
websocket_init(State) ->
    ...
    lager:debug("init beamparticle_ide_workspace_ws_handler websocket"),
    {ok, State}.
...
%% {"jsonrpc":"2.0","id":0,"method":"getRoot","params":null}
%% {"jsonrpc":"2.0","id":0,"result":"file:///opt/beamparticle-data"}
run_query(#{<<"id">> := Id,
            <<"method">> := <<"getRoot">>,
...
run_query(#{<<"id">> := Id,
            <<"method">> := <<"setRoot">>,
            <<"params">> := Uri} = _QueryJsonRpc, State) ->
    %% TODO setRoot
    ResponseJsonRpc = #{
      <<"jsonrpc">> => <<"2.0">>,
      <<"id">> => Id,
