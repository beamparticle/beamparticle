...
-module(beamparticle_dynamic_function_model).
...
%%convert_to_arguments([], AccIn) when is_binary(H) ->
%%	lists:reverse(AccIn);
%%convert_to_arguments([H | Rest], AccIn) when is_binary(H) ->
%%	convert_to_arguments(Rest, [<<"\"", H/binary, "\"">> | AccIn]);
%%convert_to_arguments([H | Rest], AccIn) when is_list(H) ->
%%	convert_to_arguments(Rest, [<<"\"", H/binary, "\"">> | AccIn]);
